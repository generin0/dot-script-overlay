...
dot_visible = True
colors = itertools.cycle(['red', 'blue', 'green', 'white']) # --> цвета для change_color()
...
def on_press(key): # --> добавляет действия по нажатию определённой клавиши
    global is_visible, x1, y1, x2, y2, current_color, crosshair_visible, dot_visible

    if key == Key.alt_l or key == Key.alt_r: # --> отрисовка прицела\точки
    ...
    elif key == Key.end: # --> показывает\скрывает прицел
        if is_visible:
            tinker.withdraw()
        else:
            tinker.deiconify()
...
def canvas_cords(): # --> обновляет размер
    canvas.coords(dot, x1, y1, x2, y2)

def keyboard_listener(): # --> слушает нажатия клавишь
...
def main(): # --> мейн функция, создание прозрачного окна, определение переменных dot, crosshair_v, crosshair_h, canvas 
    ...
    print(screen_width,"x", screen_height) # --> вывод твоего текущего разрешения экрана
    print("Press (Alt) to toggle between dot and crosshair.")
    print("Press (Delete) to change the color.")
    print("Press (Insert) to exit the overlay.")
    print("Press (End) to show or hide overlay.")
    print("Press (PAGEUP/PAGEDOWN) to increase or decrease dot size.")

    tinker.overrideredirect(True)
    tinker.attributes("-topmost", True) # --> поверх окон
    tinker.geometry(f'20x20+{screen_width//2-7}+{screen_height//2-7}') # --> размер окна процесса
    ...
    crosshair_h = canvas.create_line(0, 7, 15, 7, fill=current_color, state='hidden') # --> создание первой линии прицела
    crosshair_v = canvas.create_line(7, 0, 7, 15, fill=current_color, state='hidden') # --> создание второй линии прицела
